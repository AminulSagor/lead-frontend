...
const ProfileImageCard = ({ initialUrl }: AttachmentCardProps) => {
  const { setValue, watch } = useFormContext<B2CProfileSchemaType>();
  const imgUrl = watch("profileImgUrl");
  ...
  const handleChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setValue("profileImgUrl", publicUrl, { shouldValidate: true });
      ...
      setPreview(null);
      setValue("profileImgUrl", "", { shouldValidate: true });
    } finally {
      setUploading(false);
    }
  };
  ...
  useEffect(() => {
    if (file) {
      ...
      setValue("profileImgUrl", initialUrl, { shouldValidate: true });
    } else {
      setPreview(null);
    }
  }, [file, imgUrl, initialUrl, setValue]);
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        <div className="grid gap-2">
          <Label htmlFor="companyImgUrl">Upload Profile Image</Label>
